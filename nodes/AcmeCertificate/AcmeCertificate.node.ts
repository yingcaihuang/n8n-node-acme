...
import { HuaweiProvider } from '../../providers/HuaweiProvider';
...
export class AcmeCertificate implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			...
			{
				displayName: 'Credential Information',
				name: 'credentialInfo',
				type: 'notice',
				default: '',
				displayOptions: {
				...
				options: [
					{
						name: 'info',
						value: 'Please configure Dnspod API credentials in the Credentials section above.',
					},
				],
			},
			{
				...
				options: [
					{
						name: 'info',
						value: 'Please configure Aliyun API credentials in the Credentials section above.',
					},
				],
			},
			{
				...
				options: [
					{
						name: 'info',
						value: 'Please configure Cloudflare API credentials in the Credentials section above.',
					},
				],
			},
			{
				...
				options: [
					{
						name: 'info',
						value: 'Please configure AWS Route 53 API credentials in the Credentials section above.',
					},
				],
			},
			{
				...
				options: [
					{
						name: 'info',
						value: 'Please configure Baidu Cloud API credentials in the Credentials section above.',
					},
				],
			},
			{
				...
				options: [
					{
						name: 'info',
						value: 'Please configure Huawei Cloud API credentials in the Credentials section above.',
					},
				],
			},
			{
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (dnsProvider === 'dnspod') {
					...
					dnsProviderInstance = new BaiduProvider(
						credentials.accessKeyId as string,
						credentials.secretAccessKey as string,
					...
					dnsProviderInstance = new HuaweiProvider(
						credentials.accessKeyId as string,
						credentials.secretAccessKey as string,
						credentials.region as string || 'cn-north-1',
				...
				const options: AcmeCertificateOptions = {
					domain,
					email,
					dnsProvider: dnsProvider as 'dnspod' | 'aliyun' | 'cloudflare' | 'route53' | 'baidu' | 'huawei',
					staging,
